...
reset:
	...
	ldi     temp, high( 6250 - 1 ) ; compare value
        out     OCR1AH, temp
        ldi     temp, low( 6250 - 1 )
        out     OCR1AL, temp
...
; ERASE THIS WHEN EVERYTHING IS WORKING
; COSTS TO MUCH TIME, SAMPLING STARTS IMEDIATELY
;	ldi	data, DONE		; Send done message
;	rcall	write_byte_usb
...
read_4_channels:
	...
	cbi	PORTC, 5		; pull RD low for one cycle
...
sbi 	DDRD, 2			; PORTD 2 = output
sbi	PORTD, 5		; set it high
cbi	PORTD, 5
nop
nop
nop
...
burst_wait_for_EOLC_high:		; wait while ADC is converting
	...
	rjmp	wait_for_timer1_compare	; wait for timer1_compare to trigger next burst
